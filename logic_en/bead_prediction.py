...
# Limit OpenCV internal parallelism during batch inference.
cv2.setNumThreads(1)
import numpy as np
import torch
...
class StitchingProcessor:
    """Build 3x3 context patches and apply a fixed radial intensity weight."""
...
def load_weights(model, weight_path, device):
    """Load a checkpoint using the existing Swin key mapping."""
...
def run_prediction(image_dir, mask_dir, model_path, output_dir=None, stop_event=None, batch_size=None):
    """Classify microwell patches and save bead-positive masks."""
    ...
    print(f"Starting Swin-Transformer prediction. Device: {device}")
    ...
    print(f"Total files queued for processing: {len(image_files)}")

    for i, image_rel_path in enumerate(image_files):
        if stop_event and stop_event.is_set():
            print("🛑 Stop signal received，cancelling...")
            raise InterruptedError("Stopped by user")
        ...
        try:
            ...
            print(f"[{i + 1}/{len(image_files)}] {base_name}: "
                  f"Bead-containing wells: {bead_count} | Time: {total_s:.2f}s")
