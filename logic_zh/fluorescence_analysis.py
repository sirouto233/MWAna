...
def plot_scatter(mean_intensities, image_name, threshold, output_path):
    """保存平均强度散点图及判定阈值。"""
    plt.figure(figsize=(8, 6))
    plt.scatter(mean_intensities, range(len(mean_intensities)), color='green', alpha=0.5, s=10)
    plt.axvline(x=threshold, color='r', linestyle='--', label=f'Threshold = {threshold:.2f}')

    plt.title(f"Intensity Dist. - {image_name}")
...
def process_image(args):
    ...
    try:
        ...
        fluorescence_img = cv2.imread(fluorescence_path, cv2.IMREAD_UNCHANGED)
        if mask is None or fluorescence_img is None:
            record.update(Status='read_error', Error='Image or mask could not be read')
            return record
        if mask.ndim != 2 or fluorescence_img.ndim != 2 or mask.shape != fluorescence_img.shape:
            raise ValueError('Expected aligned single-channel image and mask with equal shapes')

        masked_img = fluorescence_img * (mask > 0)
        _, total_valid, mean_intensities = analyze_connected_components(masked_img, mask)
        ...
        # 经验灰度阈值依赖采集条件。
        THRESHOLD = 1000 if low_std > 200 else initial_threshold
        above_threshold_count = sum(1 for i in mean_intensities if i > THRESHOLD)

        f_file = os.path.basename(fluorescence_path)
        sub_output_dir = os.path.join(output_dir, subfolder_name)
        os.makedirs(sub_output_dir, exist_ok=True)
        plot_scatter(mean_intensities, f_file, THRESHOLD, sub_output_dir)

        print(f"分析完成: {f_file} (阈值: {THRESHOLD:.1f}, 计数: {above_threshold_count})")

        record.update({
            'Total Beads': total_valid,
            'Positive Beads': above_threshold_count,
            'Threshold': THRESHOLD,
            'AMB Raw': above_threshold_count / total_valid,
        })
        return record

    except Exception as e:
        print(f"处理出错 {fluorescence_path}: {e}")
        record.update(Status='analysis_error', Error=str(e))
        return record


def run_fluo_analysis(fluorescence_folder, mask_folder, z_threshold=OUTLIER_Z_THRESHOLD, min_images=OUTLIER_MIN_IMAGES):
    """配对荧光图像与含磁珠掩码，导出计数表和散点图。"""
    if not fluorescence_folder or not mask_folder:
        print("路径错误。")
        return

    parent_folder = os.path.dirname(fluorescence_folder)
    output_folder = os.path.join(parent_folder, "Analysis_Results")
    os.makedirs(output_folder, exist_ok=True)

    print(f"开始荧光分析...\n荧光图: {fluorescence_folder}\nMask: {mask_folder}")

    tasks = []
    for root, _, files in os.walk(fluorescence_folder):
        fluo_files = [f for f in files if f.endswith('-0-2.tif')]
        if not fluo_files:
            continue

        rel_path = os.path.relpath(root, fluorescence_folder)
        mask_subfolder = os.path.join(mask_folder, rel_path)
    ...
    print(f"共找到 {len(tasks)} 组配稳图像，开始处理...")
    ...
    _, summary, excluded = export_amb_reports(results, output_folder, z_threshold, min_images)
    print(f"AMB 报告已保存: {output_folder} | Excluded images: {len(excluded)}")
    return summary
